...
// import './ArcanumStyles.css';

const ArcanumCalculator = () => {
  // Estado para guardar la fecha ingresada por el usuario
  ...
  const calculateArcana = (e) => {
    ...
    // 1. Quitamos los guiones de la fecha: "1990-10-25" -> "19901025"
    ...
    while (sum > 21) {
      // Ejemplo: si sum es 27, convertimos a string "27", split ['2','7'], sumamos 2+7
      sum = sum.toString().split('').reduce((acc, curr) => acc + parseInt(curr), 0);
    }
    
    // Nota: Si la suma diera 22 exacto (algunas reglas de numerología),
    // generalmente se mapea al Loco (0).
    if (sum === 22) sum = 0;
    ...
    <div className="tarot-container">
      ...
      <form onSubmit={calculateArcana} className="tarot-form">
        ...
        <input 
          ...
          required
        />
        <button type="button" onClick={calculateArcana}>Revelar Destino</button>
        {/* <img src="./img/fondo.jpg" alt="" /> */}
</form>
      {/* Renderizado Condicional: Solo mostramos la carta si myCard existe */}
      {myCard && (
        <div className="card-display">
          <h3>Tu número de Arcanos es: {myCard.id} - {myCard.name}</h3>
